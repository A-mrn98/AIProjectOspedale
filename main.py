...
# ======================================================
# FUNZIONE GENERICA PER ESTRARRE PARAGRAFI
# ======================================================


def estrai_paragrafi_da_query(txt, query):
    """
    Restituisce tutti i paragrafi del testo che contengono una parola/frase della query.
    txt: testo completo (estratti.txt)
    query: stringa inserita dall'utente
    """
    # split del testo in paragrafi basato sui doppi a capo
    paragrafi = re.split(r'\n\s*\n', txt)
    risultati = []
    for p in paragrafi:
        # ricerca generica case-insensitive della query nel paragrafo
        if re.search(re.escape(query), p, flags=re.IGNORECASE):
...
chat_engine = index.as_chat_engine(
    ...
    system_prompt=(
        ...
        "4. Se l'utente è vago, non dare subito una rispostaa ma fai più domande di chiarimento.\n"
        "5. Non inventare mai associazioni tra servizi e numeri di edificio se non sono esplicitamente scritte nel testo recuperato."
    )
)
